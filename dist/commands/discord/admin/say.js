...
class Say extends SlashCommand_1.default {
    ...
    async run(ctx) {
        ...
        try {
            ...
            logger_1.default.info("Command", `${ctx.member.displayName}#${ctx.member.user.discriminator} said "${message}" (Server: ${server.name})`);
            await ctx.send({
                embeds: [
                    {
