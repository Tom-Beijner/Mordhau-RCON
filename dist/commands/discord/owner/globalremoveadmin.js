        ...
        try {
            Discord_1.ComponentConfirmation(ctx, {
                ...
                if (ctx.user.id !== btnCtx.user.id)
                    return;
                const result = await this.bot.rcon.globalRemoveAdmin(player);
                ...
                logger_1.default.info("Command", `${ctx.member.displayName}#${ctx.member.user.discriminator} removed ${player.name}'s global admin privileges`);
                await btnCtx.editParent({
                    embeds: [
                        {
                            description: `${allServersFailed ? "Tried to g" : "G"}lobally ${allServersFailed ? "remove" : "removed"} ${player.name} (${PlayerID_1.outputPlayerIDs(player.ids, true)})'s admin privileges`,
